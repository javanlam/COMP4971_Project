def get_query_system_prompt(member: str, rag_info: str) -> str:
    return f"""
...
You are also provided with the following information from a backend database:
{rag_info}
"""

def get_content_generation_prompt(rag_info: str) -> str:
    prompt = f"""
...
## Related Information
You are provided with the following information from a backend database:
{rag_info}
...
**DO NOT** place backticks in markdown format in your output.
...
```
"""
    return prompt + prompt_code

def get_content_regeneration_prompt(past_webpage: str, rag_info: str) -> str:
    prompt = f"""
You are a web developer. You will receive a request from a client about amending a webpage generated by a LLM. Instructions are as follows.

## Scenario
The user is browsing a webpage about the K-pop girl group (G)I-DLE. On the webpage, the user was prompted for what they want to see further.
Previously, the user has already been given a page generated based on a past request.
Now the user is asking for amendments to be made to the page.

## Related Information
You are provided with the following information from a backend database:
{rag_info}

## Previously Generated Page
...
## Your Job
The user will provide you with instructions to amend the **previously generated page**.
Follow strictly the user's instructions on amending the given webpage, by **providing ON TOP of the original content** what the user wants.
Please be sure to **change NOTHING apart from what you were instructed to do** by the user.

Your output should be an **HTML** file including **the content and design from the previous page**, with the **only difference** from the previous page being the **requested changes**. 

You are to output a **HTML** file containing **JavaScript** to be executed, in which the JavaScript code will incorporate all the frontend designs as above.
Do **NOT** use static HTML markup for content sections - all components should be created via DOM manipulation.
Try to ensure that your code is CORRECT and can be run in a browser.
